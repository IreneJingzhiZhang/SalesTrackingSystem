...
include "../inperson/QuoteEntity.php";
  ...
  //  $password = md5($user_password);
	...
	{ 
	 ...
	 if(empty($searchtext))
	 $query = "SELECT * FROM quote where status='NEW'";
	 else if (is_numeric ($searchtext))
	 $query = "SELECT * FROM quote WHERE id = ".$searchtext." and status = 'NEW'";
	 else
	 $query = "SELECT * FROM quote WHERE custname LIKE '%".$searchtext."%' and status = 'NEW'";
	 //echo $query;
	  ...
	  foreach($rows as $row)
	  {
		  ...
		  echo '<td> '. $quote->id .' </td>';
		   echo '<td> '. $quote->sid .' </td>';
			...
			echo '<td> <button class="btn btn-success" id="btnSanctionQuote" type="button" value = "'. $quote->id .'::'.$quote->sid.'">Sanction Quote</button></td>';
		  echo '</tr>';
	  }
	  echo '</tbody>';
	  echo '</table>';
